...
function addOneYearIso(value = new Date()) {
...
function getSubscriptionPeriod(user) {
  const startedAt = user?.subscriptionStartedAt || user?.createdAt || new Date().toISOString();
  const endsAt = user?.subscriptionEndsAt || addOneYearIso(startedAt);

  return {
    startedAt,
    endsAt,
    active: new Date(endsAt).getTime() >= Date.now(),
  };
}
...
export function serializeUser(user) {
  ...
  const subscriptionPeriod = getSubscriptionPeriod(user);
  ...
  return {
    ...
    vatSubscription,
    subscriptionStartedAt: subscriptionPeriod.startedAt,
    subscriptionEndsAt: subscriptionPeriod.endsAt,
    subscriptionActive: subscriptionPeriod.active,
...
subscriptionEndsAt: addOneYearIso(now),
  };
...
export async function listUsers() {
  ...
  const enrichedUsers = await Promise.all(
    users
      ...
      .map(async (user) => {
        ...
        try {
          vatCredits = await getVatCreditStatusForUser(user);
        } catch {
          vatCredits = {
            plan: normalizeVatSubscription(user.vatSubscription, user.role),
            year: String(new Date().getFullYear()),
            subscriptionStartedAt: serialized.subscriptionStartedAt,
            subscriptionEndsAt: serialized.subscriptionEndsAt,
            subscriptionActive: serialized.subscriptionActive,
            used: 0,
            limit: getVatSubscriptionLimit(user),
            unlimited: getVatSubscriptionLimit(user) === null,
            remaining: getVatSubscriptionLimit(user),
          };
        }
  ...
  return enrichedUsers.sort((a, b) => a.email.localeCompare(b.email, "en"));
}
...
export async function updateUserVatSubscriptionByEmail(email, vatSubscription = "starter") {
  ...
  user.subscriptionEndsAt = addOneYearIso(now);
  user.updatedAt = now;
...
export async function updateUserTrialByEmail(email, payload = {}) {
  const normalizedEmail = normalizeEmail(email);
  ...
  if (user.role === "admin") {
    throw new Error("Trial is not applicable for admin users");
  }

  user.isTrial = Boolean(payload.isTrial);
  user.trialEndsAt = user.isTrial ? String(payload.trialEndsAt || "").trim() : "";
  user.updatedAt = new Date().toISOString();
...
function secondsUntilSubscriptionEnds(user) {
  const period = getSubscriptionPeriod(user);
  const endsAt = new Date(period.endsAt).getTime();

  if (!Number.isFinite(endsAt)) return 60 * 60 * 24 * 365;
...
export async function consumeVatCreditsForUser(user, count) {
  ...
  await redis.expire(key, secondsUntilSubscriptionEnds(user));

  if (limit !== null && after > limit) {
    await redis.incrby(key, -amount);

    const used = Number(await redis.get(key)) || 0;
    const status = await getVatCreditStatusForUser(user);
...
export async function resetVatCreditsForUserByEmail(email) {
  const normalizedEmail = normalizeEmail(email);
